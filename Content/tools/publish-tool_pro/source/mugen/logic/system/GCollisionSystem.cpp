...
bool GCollisionSystem::test(GActorComponent* actorA, GActorComponent* actorB)
{
	...
	auto pAvatarComponentA = G_GetComponent(actorA, GAvatarComponent);
	auto pAvatarComponentB = G_GetComponent(actorB, GAvatarComponent);

	auto pStaticDataComponentA = G_GetComponent(actorA, GStaticDataComponent);
	auto pStaticDataComponentB = G_GetComponent(actorB, GStaticDataComponent);

	auto& renderPosA = pAvatarComponentA->getRenderPos();
	auto& renderPosB = pAvatarComponentB->getRenderPos();

	auto minA = renderPosA.y - pStaticDataComponentA->radius;
	auto maxA = renderPosA.y + pStaticDataComponentA->radius;
	auto minB = renderPosB.y - pStaticDataComponentB->radius;
	auto maxB = renderPosB.y + pStaticDataComponentB->radius;
