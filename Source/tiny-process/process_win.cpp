...
#    endif
    ...
    BOOL bSuccess =
        CreateProcess(nullptr, process_command.empty() ? nullptr : &process_command[0], nullptr, nullptr, TRUE, 0,
...
void Process::async_read() noexcept
{
    ...
    if (stdout_fd)
    {
        stdout_thread = std::thread([this]() {
            ...
            for (;;)
            {
                BOOL bSuccess = ReadFile(*stdout_fd, static_cast<CHAR*>(buffer.get()), static_cast<DWORD>(buffer_size),
                                         &n, nullptr);
                if (!bSuccess || n == 0)
                    break;
    ...
    if (stderr_fd)
    {
        stderr_thread = std::thread([this]() {
            ...
            for (;;)
            {
                BOOL bSuccess = ReadFile(*stderr_fd, static_cast<CHAR*>(buffer.get()), static_cast<DWORD>(buffer_size),
                                         &n, nullptr);
                if (!bSuccess || n == 0)
                    break;
