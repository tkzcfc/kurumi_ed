...
function push_cc_class_name(className)
    ...
    func = string.gmatch(className, "ccs.(%w+)")
    if func then
        local ccName = func()
        if ccName and ccName ~= "" then
            cc_class_name_tab["ccs." .. ccName] = ccName
        end
    end
end
        ...
        if func == "tolua_pushusertype" then
            ...
            if templateContainer and templateContainer ~= "" then
                ...
                elseif templateContainer == "map" then
                    ...
                    elseif arg1 == "string" or  arg1 == "int" then
                        return "tolua_ext_map_int_object_to_luaval<" .. arg2 .. ">"
                    end
                end
            end
        end
...
_push_functions["LUA_FUNCTION"] = "toluafix_totable"
...
function post_output_hook(package)
    ...
    replace("\"cc_Node\"", "\"cc.Node\"")
    replace("(cc_Node*)", "(Node*)")

    replace("<ccs_Armature>", "<Armature>")
    -- replace("<ccs.Armature>", "<Armature>")
    replace("\"ccs_Armature\"", "\"ccs.Armature\"")
    replace("(ccs_Armature*)", "(Armature*)")
    -- replace("(ccs.Armature*)", "(Armature*)")
